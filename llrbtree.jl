...
function draw(coords)
    ...
    for i in 1:width
        ...
        circle_ = treecircle(x, y, 1/width, string(coords[i][4]), color )
...
draw(col::ColumnFull) = draw(getcoords(col))
draw(tree::LLRBTree) = draw(buildcolumns(tree))

function treecircle(x::Float64, y::Float64, side::Float64, value::String, color::String)
    compose(context(x, y, side, side), circle(), fill(color), (context(0,0), text(0.2,0.55,value), fill("white")))
end
